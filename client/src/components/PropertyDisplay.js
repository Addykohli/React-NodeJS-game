...
const PropertyDisplayLeft = ({ player, position }) => {
  ...
  return (
    <div style={{
      ...
      transform: `translate(5px, calc(${position === 'top' ? '-50%' : '-135%'} + ${centerOffset}px - ${(ownedProperties.length - 1) * cardOverlap / 2}px ${position === 'top' ? '- 120px' : '+ 200px'}))`,
      width: cardWidth + 'px',
      display: 'flex',
      alignItems: 'flex-start',
      overflow: 'visible',
...
const PropertyDisplayRight = ({ player, position }) => {
  ...
  return (
    <div style={{
      ...
      transform: `translate(-5px, calc(${position === 'top' ? '-50%' : '-75%'} + ${centerOffset}px - ${(ownedProperties.length - 1) * cardOverlap / 2}px ${position === 'top' ? '- 30px' : '+ 200px'}))`,
      width: cardWidth + 'px',
      display: 'flex',
      alignItems: 'flex-start',
      overflow: 'visible',
